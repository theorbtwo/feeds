...
sub issued {
    ...
    if (@_) {
      ...
      for my $child ($entry->{entry}{elem}->getChildNodes) {
        next if $child->isa('XML::XPath::Node::TextImpl');

        if ($child->getLocalName eq 'published') {
          return iso2dt($child->string_value);
        }
      }

      # --- I am lost in a twisty maze with the stuff below.

      if ($entry->{entry}->issued) {
        return iso2dt($entry->{entry}->issued);
      } elsif ($entry->{entry}->published) {
