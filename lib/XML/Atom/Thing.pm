...
sub fixup_ns_xpath {
    ...
    if (not $obj->elem->getNamespace('#default')) {
      use Carp 'cluck';
      cluck('fixup_ns_xpath with no #default namespace');
    }

    $obj->{ns} = $obj->elem->getNamespace('#default')->getExpanded;
    #warn "New ns is $obj->{ns}";
}
...
sub _rename_elements {
    ...
    *{"$class\::$atom03"} = sub {
        ...
        @_ > 0 ? $self->set($self->ns, $atom03, @_) : $self->get($XML::Atom::Util::NS_MAP{'0.3'}, $atom03);
    };
    ...
    *{"$class\::$atom10"} = sub {
        ...
        @_ > 0 ? $self->set($self->ns, $atom10, @_) : $self->get($XML::Atom::Util::NS_MAP{'1.0'}, $atom10);
    };
}
