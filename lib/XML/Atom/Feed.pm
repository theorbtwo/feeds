...
sub get_namespaces {
  ...
  my %namespaces;
  if ($e->getParentNode) {
    my %x = get_namespaces($e->getParentNode);
    $namespaces{$_} = $x{$_} for keys %x;
  }
  for my $ns ($e->getNamespaces) {
    $namespaces{$ns->getPrefix} = $ns->getExpanded;
  }
  return %namespaces;
}
...
sub entries_xpath {
    ...
    for my $elem ($set->get_nodelist) {
        # Find the namespaces from our parents.
        my %namespaces = get_namespaces($elem->getParentNode);
        
        # delete the link to ou parent.
        $elem->del_parent_link;

        # Add the namespaces from the parent back in.
        for my $prefix (keys %namespaces) {
            my $uri = $namespaces{$prefix};
            #print "$prefix => $uri\n";
            
            $elem->appendNamespace(XML::XPath::Node::Namespace->new($prefix => $uri));
        }
